...
export default function SalonUpdate() {
  ...
  return (
    <motion.section initial={{ opacity: 0, y: 8 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mx-auto w-full max-w-md pb-3 pt-1">
      <Link href={index === 2 ? '/book-consultation' : '/packages'} className="group relative flex h-[126px] overflow-hidden rounded-[25px] border border-[#d8be80]/55 bg-[#604b22] shadow-[0_14px_30px_rgba(0,0,0,.24)]">
        ...
        <AnimatePresence mode="wait"><motion.div key={update.title} initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }} transition={{ duration: .28 }} className="relative z-10 flex h-full w-[78%] flex-col justify-center px-4 pb-7 pr-9 pt-3 text-white"><span className="mb-1 inline-flex w-fit items-center gap-1 rounded-full border border-white/25 bg-white/12 px-2 py-1 text-[9px] font-black uppercase tracking-[.13em] backdrop-blur-md"><Sparkles className="h-3 w-3" /> {update.badge}</span><h2 className="line-clamp-1 text-[17px] font-black leading-tight">{update.title}</h2><p className="mt-1 line-clamp-2 text-[11px] font-semibold leading-4 text-white/78">{update.copy}</p></motion.div></AnimatePresence>
